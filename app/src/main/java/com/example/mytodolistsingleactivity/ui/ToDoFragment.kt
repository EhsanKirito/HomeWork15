...
class ToDoFragment : Fragment(R.layout.fragment_to_do) {
    ...
    fun recyclerViewInitiator(){
        ...
        val adaptor = ToDoAdaptor2(
            ...
            txtName.setText(viewModel.taskArray[viewModel.getPos(position, TaskStatus.Todo)].taskName)
            txtDescription.setText(viewModel.taskArray[viewModel.getPos(position, TaskStatus.Todo)].taskdescription)
            btnTime.text = viewModel.taskArray[viewModel.getPos(position, TaskStatus.Todo)].taskTime
            btnDate.text = viewModel.taskArray[viewModel.getPos(position, TaskStatus.Todo)].taskDate
            when(viewModel.taskArray[viewModel.getPos(position, TaskStatus.Todo)].taskStatus){
            ...
            btnDelete.setOnClickListener {
                viewModel.deleteTask(viewModel.repository.get()[(viewModel.getPos(position, TaskStatus.Todo))])
                alertDialog.dismiss()
            }
        }
        ...
        viewModel.taskListToDo.observe(viewLifecycleOwner, Observer {
            adaptor.submitList(viewModel.taskListToDo.value)
            recyclerView.adapter = adaptor
            if (viewModel.taskListToDo.value?.isEmpty() == true){

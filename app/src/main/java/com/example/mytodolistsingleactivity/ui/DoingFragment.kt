...
class DoingFragment : Fragment(R.layout.fragment_doing) {
    ...
    fun recyclerViewInitiator(){
        ...
        val adaptor = ToDoAdaptor2(
            ...
            txtName.setText(viewModel.taskArray[viewModel.getPos(position, TaskStatus.Doing)].taskName)
            txtDescription.setText(viewModel.taskArray[viewModel.getPos(position, TaskStatus.Doing)].taskdescription)
            btnTime.text = viewModel.taskArray[viewModel.getPos(position, TaskStatus.Doing)].taskTime
            btnDate.text = viewModel.taskArray[viewModel.getPos(position, TaskStatus.Doing)].taskDate
            when(viewModel.taskArray[viewModel.getPos(position, TaskStatus.Doing)].taskStatus){
            ...
            btnDelete.setOnClickListener {
                viewModel.deleteTask(viewModel.repository.get()[(viewModel.getPos(position, TaskStatus.Doing))])
                alertDialog.dismiss()
            }
        }
        ...
        viewModel.taskListDoing.observe(viewLifecycleOwner) {
            adaptor.submitList(viewModel.taskListDoing.value)
            recyclerView.adapter = adaptor
            if (viewModel.taskListDoing.value?.isEmpty() == true) {
